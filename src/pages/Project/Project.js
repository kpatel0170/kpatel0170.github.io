          ...
          <div className="flex space-x-4 mt-4">
            {project.links.map((link, i) => (
              <a
                key={i}
                href={link.url}
                className="inline-flex items-center px-4 py-2 border border-transparent text-sm leading-5 font-medium rounded-md text-white bg-blue-500 hover:bg-blue-600 focus:outline-none focus:border-blue-700 focus:shadow-outline-blue active:bg-blue-700 transition duration-150 ease-in-out"
              >
                <svg
      ...
      <div className="space-y-8">
        {projects.projects.list.map((project, index) => (
          <div key={index} className="relative">
            ...
            {index !== projects.projects.list.length - 1 && (
